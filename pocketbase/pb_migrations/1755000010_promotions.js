...
migrate((app) => {
  ...
  const collection = new Collection({
    ...
    fields: [
      ...
      new Field({ name: "start_at", type: "date", required: true }),
      new Field({ name: "end_at", type: "date", required: true }),
      new Field({ name: "top_position", type: "number", min: 1, max: 20, onlyInt: true }),
      new Field({ name: "priority", type: "number", onlyInt: true }),
      new Field({ name: "rotation_enabled", type: "bool" }),
      new Field({ name: "impressions", type: "number", min: 0, onlyInt: true }),
      new Field({ name: "clicks", type: "number", min: 0, onlyInt: true }),
      new Field({ name: "leads", type: "number", min: 0, onlyInt: true }),

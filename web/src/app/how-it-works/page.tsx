  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        answer:
          "Заявка появляется в кабинете специалиста, и дальше всё общение идёт прямо на площадке — в разделе «Мои заявки» открывается чат с историей переписки. Когда договоритесь об условиях, одна из сторон нажимает «Предложить сделку» и указывает результат, стоимость и срок, вторая подтверждает — и заказ переезжает в раздел «Заказы».",
      },
      {
        question: "Как проходит сам заказ после того, как сделка заключена?",
        answer:
          "В разделе «Заказы» видна согласованная стоимость и срок сдачи с обратным отсчётом дней. Когда специалист выполнит работу, он нажимает «Услуга оказана» — у заказчика есть время проверить результат. Если всё устраивает, заказ уходит в «Архив заказов». Если нужны правки — заказчик нажимает «Требуется доработка», и стороны возвращаются в чат обсудить детали; специалист снова отмечает «Услуга оказана», когда доработает — и так до тех пор, пока заказчик не будет доволен, без необходимости заново заключать сделку.",
      },
      {
        question: "Что делать, если возник спор с исполнителем или заказчиком?",
        answer:
          "В любой момент активного заказа доступна кнопка «Жалоба модераторам» — она сразу уведомляет команду площадки, и модератор подключается к вашему чату как третья сторона. Он может либо закрыть сделку своим решением, либо отправить заказ на доработку — в зависимости от того, что справедливо в конкретной ситуации.",
      },
      {
